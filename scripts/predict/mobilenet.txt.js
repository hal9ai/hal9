...
if (url && data) {
  var state = hal9.getState() ? hal9.getState() : {};
  ...
  for (var idx = 0; idx < data.length; idx++) {
    ...
    if (state[dataurl]) {
      ...
      const input = tf.cast(tf.browser.fromPixels(image).expandDims(0), 'float32').div(255);
      const predictions = model.predict(input);

      var end = performance.now()

      console.log("Prediction " + (end - start) + " milliseconds.")

      const prediction = (await predictions.argMax(1).data())[0];;

      const regex = /,.*/i;
      Object.keys(IMAGENET_CLASSES).map(e => { IMAGENET_CLASSES[e] = IMAGENET_CLASSES[e].replace(regex, '') });

      data[idx] = Object.assign({ prediction: IMAGENET_CLASSES[prediction] }, data[idx])
      state[dataurl] = data[idx]
    }
  }
