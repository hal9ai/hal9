...
Hal9's Enterprise plan is recommended if:
...
This plan enable your business users to answer data questions and find insights in seconds inside their own infrastructure with Hal9's conversational AI.
...
Hal9's LLM API lives in hal9.com and it is the only component that will be outside the enterprise. As we mention, we never take a look at your data and we only use the metadata for this data (table names, table description and the input prompt). This is usually more than enough and sufficient for many of the organizations but if you happen to be in a case where the application you use cannot even access to the internet, talk to us, we have a workaround to also run the model that we are using within your organization but it is quite complex and in most cases it is not necessary at all.
