...
Deploy applications to the Hal9 server.

### Deploy
`deploy(path, target, url, name, typename, data, access, main, title, description)` <br/><br/>
Deploys an application to Hal9. Requires a `HAL9_TOKEN` environment variable from [hal9.com/devs](https://hal9.com/devs).
...
| --- | --- | --- |
| path | <code>String</code> | Path to the application. Defaults to the current directory. |
| target | <code>String</code> | The deployment target, defaults to `hal9`. |
| url | <code>String</code> | The server url to deploy to. Defaults to `https://api.hal9.com`. |
| name | <code>String</code> | The deployment name, automatically generated by default. |
| typename | <code>String</code> | The type of content to deploy. Defaults to (chatbot) `ability`. |
| data | <code>String</code> | The data schema to use, defaults to empty. |
| access | <code>String</code> | The access level, defaults to `private`. |
| main | <code>String</code> | The main file to use, defaults to `app.py`. |
| title | <code>String</code> | The deployment title, defaults to name. |
| description | <code>String</code> | The deployment description, defaults to empty. |
